...
from processtrad import Informationsobjekt, Arkiv, Verksamhetsomrade, Processgrupp, Process, informationsobjekt_to_markdown
...
class testInformationsobjekt(unittest.TestCase):
    ...
    def test_processgrupp_to_md(self):
        processgrupp = Processgrupp(name="Styrande", num="1.1", description="test", verksamhetsomrade="test")
        md = processgrupp.to_markdown()
        self.assertEqual(md, "## 1.1 Styrande\n" + date.today().strftime('%Y-%m-%d') + " -")

    def test_process_to_md(self):
        process = Process(name="Styrande", num="1.1.1.", description="test", processgrupp="test", forvaring="test")
